...
def QH_entro(top_file,traj_file,index_file,temperature=300,skip=1,begin=0,end=-1,CYCLES=1):
    '''
    Add some words here.
    '''
...
##########################################
    ...
    for cycle in range(CYCLES):
        cycle_frame = nframes/CYCLES*(cycle+1)
...
def CheckArgs():
    if len(sys.argv) > 1:
        ...
        if os.path.isfile(traj_file) and os.path.isfile(top_file) and os.path.isfile(index_file):
            QH_entro(top_file,traj_file,index_file,temperature,skip,begin_time,end_time,cycles)
    else:
        Usage()
        sys.exit()
